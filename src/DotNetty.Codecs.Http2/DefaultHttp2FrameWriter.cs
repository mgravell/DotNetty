 ...
 */
    ...
    {
        static readonly string STREAM_ID = "Stream ID";

        static readonly string STREAM_DEPENDENCY = "Stream Dependency";
         ...
         */
        static readonly IByteBuffer ZERO_BUFFER =
            Unpooled.UnreleasableBuffer(Unpooled.DirectBuffer(Http2CodecUtil.MAX_UNSIGNED_BYTE).WriteZero(Http2CodecUtil.MAX_UNSIGNED_BYTE));
        ...
        public DefaultHttp2FrameWriter(SensitivityDetector headersSensitivityDetector, bool ignoreMaxHeaderListSize)
        ...
        public Task writeData(IChannelHandlerContext ctx, int streamId, IByteBuffer data, int padding, bool endStream, IPromise promise)
        {
            ...
            try
            {
                ...
                if (remainingData > this._maxFrameSize)
                {
                    frameHeader = ctx.Allocator.Buffer(Http2CodecUtil.FRAME_HEADER_LENGTH);
                    Http2CodecUtil.writeFrameHeaderInternal(frameHeader, this._maxFrameSize, Http2FrameTypes.DATA, flags, streamId);
                    do
                    {
                        // Write the header.
                        ...
                        // Write the payload.
                        ctx.WriteAsync(data.ReadRetainedSlice(this._maxFrameSize), promiseAggregator.newPromise());

                        remainingData -= this._maxFrameSize;
                        // Stop iterating if remainingData ==  _maxFrameSize so we can take care of reference counts below.
                    }
                    while (remainingData > this._maxFrameSize);
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (frameHeader == null)
                        {
                            ...
                            Http2CodecUtil.writeFrameHeaderInternal(lastFrame, this._maxFrameSize, Http2FrameTypes.DATA, flags, streamId);
                        }
                        else
                        {
                        ...
                        // Write the payload.
                        lastFrame = data.ReadSlice(this._maxFrameSize);
                        data = null;
                        ctx.WriteAsync(lastFrame, promiseAggregator.newPromise());
                    }
                    ...
                    do
                    {
                        int frameDataBytes = Math.Min(remainingData, this._maxFrameSize);
                        int framePaddingBytes = Math.Min(padding, Math.Max(0, (this._maxFrameSize - 1) - frameDataBytes));
        ...
        public Task writeHeaders(
            ...
            IPromise promise)
        {
            return this.writeHeadersInternal(ctx, streamId, headers, padding, endStream, false, 0, 0, false, promise);
        }
        ...
        public Task writePriority(IChannelHandlerContext ctx, int streamId, int streamDependency, short weight, bool exclusive, IPromise promise)
        {
            try
            {
                verifyStreamId(streamId, STREAM_ID);
        ...
        public Task writeRstStream(IChannelHandlerContext ctx, int streamId, long errorCode, IPromise promise)
        {
            try
            {
                verifyStreamId(streamId, STREAM_ID);
        ...
        public Task writeSettings(IChannelHandlerContext ctx, Http2Settings settings, IPromise promise)
        {
            try
            {
        ...
        public Task writeSettingsAck(IChannelHandlerContext ctx, IPromise promise)
        {
            try
            {
        ...
        public Task writePing(IChannelHandlerContext ctx, bool ack, IByteBuffer data, IPromise promise)
        {
            SimplePromiseAggregator promiseAggregator = new SimplePromiseAggregator(promise);
            try
            {
            ...
            IPromise promise)
        {
            ...
            try
            {
                ...
                int maxFragmentLength = this._maxFrameSize - nonFragmentLength;
        ...
        public Task writeGoAway(
            IChannelHandlerContext ctx,
            int lastStreamId,
            long errorCode,
            IByteBuffer debugData,
            IPromise promise)
        {
            SimplePromiseAggregator promiseAggregator = new SimplePromiseAggregator(promise);
            try
            ...
            IPromise promise)
        {
            ...
            try
            {
                ...
                int maxFragmentLength = this._maxFrameSize - nonFragmentBytes;
                IByteBuffer fragment = headerBlock.ReadRetainedSlice(Math.Min(headerBlock.ReadableBytes, maxFragmentLength));
        ...
        {
            Http2Flags flags = new Http2Flags().paddingPresent(padding > 0);
            int maxFragmentLength = this._maxFrameSize - padding;
            // TODO: same padding is applied to all frames, is this desired?
            if (maxFragmentLength <= 0)
            {
                promiseAggregator.SetException(new ArgumentException("Padding [" + padding + "] is too large for max frame size [" + this._maxFrameSize + "]"));
                return promiseAggregator.Task;
            }
        ...
        static void verifyStreamId(int streamId, string argumentName)
        {
            if (streamId <= 0)
            {
