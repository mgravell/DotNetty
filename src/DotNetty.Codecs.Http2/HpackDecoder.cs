...
namespace DotNetty.Codecs.Http2
{
    ...
    {
        static readonly Http2Exception DECODE_ULE_128_DECOMPRESSION_EXCEPTION = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - decompression failure"), typeof(HpackDecoder),
        "decodeULE128(..)");

    static readonly Http2Exception DECODE_ULE_128_TO_LONG_DECOMPRESSION_EXCEPTION = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - long overflow"), typeof(HpackDecoder), "decodeULE128(..)");

    static readonly Http2Exception DECODE_ULE_128_TO_INT_DECOMPRESSION_EXCEPTION = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - int overflow"), typeof(HpackDecoder), "decodeULE128ToInt(..)");

    static readonly Http2Exception DECODE_ILLEGAL_INDEX_VALUE = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - illegal index value"), typeof(HpackDecoder), "decode(..)");

    static readonly Http2Exception INDEX_HEADER_ILLEGAL_INDEX_VALUE = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - illegal index value"), typeof(HpackDecoder), "indexHeader(..)");

    static readonly Http2Exception READ_NAME_ILLEGAL_INDEX_VALUE = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - illegal index value"), typeof(HpackDecoder), "readName(..)");

    static readonly Http2Exception INVALID_MAX_DYNAMIC_TABLE_SIZE = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - invalid max dynamic table size"), typeof(HpackDecoder),
        "setDynamicTableSize(..)");

    static readonly Http2Exception MAX_DYNAMIC_TABLE_SIZE_CHANGE_REQUIRED = unknownStackTrace(
        Http2Exception.connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - max dynamic table size change required"), typeof(HpackDecoder),
        "decode(..)");

    static readonly byte READ_HEADER_REPRESENTATION = 0;

    static readonly byte READ_MAX_DYNAMIC_TABLE_SIZE = 1;

    static readonly byte READ_INDEXED_HEADER = 2;

    static readonly byte READ_INDEXED_HEADER_NAME = 3;

    static readonly byte READ_LITERAL_HEADER_NAME_LENGTH_PREFIX = 4;

    static readonly byte READ_LITERAL_HEADER_NAME_LENGTH = 5;

    static readonly byte READ_LITERAL_HEADER_NAME = 6;

    static readonly byte READ_LITERAL_HEADER_VALUE_LENGTH_PREFIX = 7;

    static readonly byte READ_LITERAL_HEADER_VALUE_LENGTH = 8;

    static readonly byte READ_LITERAL_HEADER_VALUE = 9;

    readonly HpackDynamicTable hpackDynamicTable;

    readonly HpackHuffmanDecoder hpackHuffmanDecoder;

    readonly long maxHeaderListSizeGoAway;
    long maxHeaderListSize;
    long maxDynamicTableSize;
    long encoderMaxDynamicTableSize;
    readonly bool maxDynamicTableSizeChangeRequired;
...
throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Header Table Size must be >= %d and <= %d but was %d",
...
throw Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, "Header List Size GO_AWAY %d must be positive and >= %d",
...
throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Header List Size must be >= %d and <= %d but was %d",
...
private CharSequence readName(int index) {
...
IndexType indexType, long headerSize) {
...
private CharSequence readStringLiteral(ByteBuf in, int length, bool huffmanEncoded) {
if (huffmanEncoded) {
...
final bool resultStartedAtZero = result == 0;
