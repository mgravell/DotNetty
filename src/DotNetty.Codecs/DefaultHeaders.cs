    ...
    public class DefaultHeaders<TKey, TValue> : IHeaders<TKey, TValue>
    {
            ...
            IValueConverter<TValue> valueConverter, INameValidator<TKey> nameValidator, int arraySizeHint)
        {
            ...
            this.entries = new HeaderEntry<TKey, TValue>[
                MathUtil.SafeFindNextPositivePowerOfTwo(Math.Max(2, Math.Min(arraySizeHint, 128)))];
        ...
        public IHeaders<TKey, TValue> AddEntry(TKey name, TValue value)
        {
            int h = this.hashingStrategy.GetHashCode(name);
            int i = this.IndexOf(h);
        ...
        public IEnumerator<HeaderEntry<TKey, TValue>> GetEnumerator() => new HeaderEnumerator(this.head);
        ...
        {
            return obj is IHeaders<TKey, TValue> headers 
        ...
        public int HashCode(IHashingStrategy<TValue> valueHashingStrategy)
        {
            ...
            foreach (TKey name in this.Names())
            {
                ...
                {
                    result = 31 * result + valueHashingStrategy.GetHashCode(value);
                }
            }
        ...
        sealed class HeaderEnumerator : IEnumerator<HeaderEntry<TKey, TValue>>
        {
            HeaderEntry<TKey, TValue> head;
            HeaderEntry<TKey, TValue> current;

            public HeaderEnumerator(HeaderEntry<TKey, TValue> head)
            {
                this.head = head;
            }

            public HeaderEntry<TKey, TValue> Current => this.current;

            object IEnumerator.Current => this.Current;

            public bool MoveNext()
            {
                this.current = this.current == null ? this.head.After : this.current.After;
