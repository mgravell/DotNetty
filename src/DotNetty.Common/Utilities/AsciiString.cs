...
namespace DotNetty.Common.Utilities
{
    ...
    public sealed class AsciiString : ICharSequence, IEquatable<AsciiString>, IComparable<AsciiString>, IComparable
    {
        ...
        public AsciiString(byte[] value, bool copy = true) : this(value, 0, value.Length, copy)
        {
        }
        ...
        internal AsciiString(char[] value) : this(value, 0, value.Length)
        {
        }

        internal unsafe AsciiString(char[] value, int start, int length)
        {
            if (MathUtil.IsOutOfBounds(start, length, value.Length))
            {
                throw new IndexOutOfRangeException($"expected: 0 <= start({start}) <= start + length({length}) <= value.length({value.Length})");
        ...
        public AsciiString(ICharSequence value, int start, int length)
        {
            ...
            {
                throw new IndexOutOfRangeException($"expected: 0 <= start({start}) <= start + length({length}) <= value.length({value.Count})");
            }
        ...
        public int ForEachByte(int index, int count, ByteProcessor visitor)
        {
            if (MathUtil.IsOutOfBounds(index, count, this.length))
            {
                throw new IndexOutOfRangeException($"expected: 0 <= index({index} <= start + length({count}) <= length({this.length})");
            }

            return this.ForEachByte0(index, count, visitor);
        }

        int ForEachByte0(int index, int count, ByteProcessor visitor)
        {
        ...
        public int ForEachByteDesc(int index, int count, ByteProcessor visitor)
        {
            if (Internal.MathUtil.IsOutOfBounds(index, count, this.length))
            {
                throw new IndexOutOfRangeException($"expected: 0 <= index({index} <= start + length({count}) <= length({this.length})");
        ...
        int ForEachByteDesc0(int index, int count, ByteProcessor visitor)
        {
        ...
        public bool RegionMatches(bool ignoreCase, int thisStart, ICharSequence seq, int start, int count)
        {
            Contract.Requires(seq != null);

            if (!ignoreCase)
            {
                return this.RegionMatches(thisStart, seq, start, count);
            }
        ...
        {
            List<ICharSequence> res = InternalThreadLocalMap.Get().CharSequenceList();
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        int i = res.Count - 1;
                        if (!(res[i] is AsciiString asciiString) || asciiString.IsEmpty)
                        {
                            res.RemoveAt(i);
                        }
                        else
        ...
        public unsafe string ToString(int start, int end)
        {
            ...
            fixed (char* chars = data)
                fixed (byte* bytes = this.value)
                    GetChars(bytes + this.offset + start, count, chars);
        ...
        public static int GetHashCode(ICharSequence value)
        {
            ...
            return value is AsciiString asciiString
        ...
        public static bool RegionMatches(ICharSequence cs, bool ignoreCase, int csStart, ICharSequence seq, int start, int length)
        {
            ...
            {
                return stringCharSequence.RegionMatches(ignoreCase, csStart, seq, start, length);
            }
        ...
        static unsafe void GetChars(byte* bytes, int length, char* chars)
        {
            byte* byteEnd = bytes + length;
            while (bytes < byteEnd)
            {
                *(chars++) = (char)*(bytes++);
            }
        ...
        public bool SequenceEquals(ICharSequence other, bool ignoreCase) => 
            other != null && ignoreCase ? this.ContentEqualsIgnoreCase(other) : this.ContentEquals(other);
